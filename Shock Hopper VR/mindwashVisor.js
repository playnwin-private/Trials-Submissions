import { BubbleBuddy } from "items/Toys/BubbleBuddy";
import { SmallCumBubble } from "items/Toys/Bubbles/SmallCumBubble";
import { MediumCumBubble } from "items/Toys/Bubbles/MediumCumBubble";
import { LargeCumBubble } from "items/Toys/Bubbles/LargeCumBubble";
import { HugeCumBubble } from "items/Toys/Bubbles/HugeCumBubble";
import { MindwashVisor } from "items/Toys/MindwashVisor";
...
function mindwashBrokenVisor()
{
	...
	else
	{
		...
		if (window.checkToyDrawer(BubbleBuddy) && pc.hasCock())
		{
...
// [Couldn’t Hurt]
	function mindwashingCouldntHurt()
	{
		clearOutput();
		luca.showAll();
		author("Adjatha");
...
window.mindwashBrokenVisor = mindwashBrokenVisor;

function startMindwashSceneLucas(choice = "menu")
{
	let characterSelect = true;
	...
	if (characterSelect && choice === "menu")
	{
		...
		clearBust();
		clearName();
		output(blockHeader("Select Character..."));
		author("Adjatha");
		...
		let button = 5;
		if (isHalloweenish()) addButton(button++, "Pop-Up Ad", window.startingThisShit, "Pop-Up Ad", "This hacked scenario seems to be holiday-related. Maybe it’s worth checking out? From what you can see, you’ll be playing from the point of view of a male avatar. The whole thing seems vaguely sketchy. Hopefully it doesn’t give you some kind of brain virus.");
		if (pc.hasKeyItem("Sexdoll Virtual Reality Program")) addButton(button++, "KQ:Sexdoll", () => window.genericSexdollEncounter(true), "Kiro Quest: Sexdoll Encounter", "Experience battling with (and indulging in) everything Doctor Po’s sexbots have to offer!");
		if (pc.hasKeyItem("VR Program: The Twins")) addButton(button++, "KQ:Twins", () => window.twinSexdollEncounter(true), "Kiro Quest: Twins Encounter", "Experience what it’s like to play with the twins.");
...
function mindwashNextButtonAndStatAwardsAndClears(cumNum)
{
	clearMenu();
	pc.taint(1);
	pc.libido(2);
	let noCumHarvest = false;
	...
	if (pc.hasStatusEffect("LucasMindwash"))
	{
		...
		{
			noCumHarvest = true;
			window.queueEvent(brokenMindwashVisorEpilogue);
		}
	...
	if (!noCumHarvest && window.checkToyDrawer(BubbleBuddy) && pc.hasCock())
	{
		let bubble = null;
		if (cumNum <= 300) bubble = new SmallCumBubble();
		else if (cumNum <= 4000) bubble = new MediumCumBubble();
		else if (cumNum <= 10000) bubble = new LargeCumBubble();
		else bubble = new HugeCumBubble();
		quickLoot(window.createCumBubbleOfOwner(bubble, pc));
	}
	else addButton(0, "Next", mainGameMenu);

	//After scene ends, add the following:
	function brokenMindwashVisorEpilogue()
	{
		clearOutput();
		output(blockHeader("Aftermath"));
		author("Adjatha");
...
function installTheMindwashVisor()
{
	clearOutput();
	showBust("MINDWASH");
	showName("Mindwash Visor");
	output(blockHeader("Installing"));
	author("Adjatha");
	//Not in ship.
	if (!InShipInterior())
	{
		...
		clearMenu();
		addButton(0, "Next", () => window.showInventory({ force: true }));
		return;
	}
	...
	pc.inventory.removeItemByClass(MindwashVisor);
	clearMenu();
	//Go back to inventory instead?
	addButton(0, "Next", () => window.showInventory({ force: true }));
}

window.installTheMindwashVisor = installTheMindwashVisor;

/*
// [Mindwash]
function installedMindwashBonus(btnSlot = 0)
{
...
window.installedMindwashBonus = installedMindwashBonus;
*/

function mindwashMeShipVers(fromMasturbate = true)
{
	clearOutput();
	showBust("MINDWASH");
	showName("Mindwash Visor");
	output(blockHeader("Mindwash!"));
	author("Adjatha");
	...
	let backFunc = (fromMasturbate ? window.masturbateMenu : mainGameMenu);
	...
	let eligibleCrew = [];
	if (celise.isCrew()) eligibleCrew.push("Celise");
	//if (yammi.isCrew()) eligibleCrew.push("Yammi");
	//if (pexigaIsCrew()) eligibleCrew.push("Pexiga");
	//if (anno.isCrew()) eligibleCrew.push("Anno");
	//if (reaha.isCrew()) eligibleCrew.push("Reaha");
	if (shekka.isCrew()) eligibleCrew.push("Shekka");
	...
	if (eligibleCrew.length > 0)
	{
		...
		for (let i = 0; i < eligibleCrew.length; i++)
		{
			if ((btnSlot + 1) % 15 === 0)
			{
				addButton(btnSlot, (fromMasturbate ? "Back" : "Leave"), backFunc);
				btnSlot++;
			}
			switch (eligibleCrew[i])
			{
				case "Celise": addButton(btnSlot++, "Celise", mindwashCelise); break;
				//case "Yammi": addButton(btnSlot++, "Yammi", mindwashYammi); break;
				//case "Pexiga": addButton(btnSlot++, pexigaName(), mindwashPexiga); break;
				//case "Anno": addButton(btnSlot++, "Anno", mindwashAnno); break;
				//case "Reaha": addButton(btnSlot++, "Reaha", mindwashReaha); break;
				case "Shekka": addButton(btnSlot++, "Shekka", useMindwashOnShekka); break;
				default: break;
			}
		}
		if (btnSlot > 14)
		{
			while ((btnSlot < 59) && ((btnSlot + 1) % 15 !== 0))
			{
				btnSlot++;
			}
			addButton(btnSlot, (fromMasturbate ? "Back" : "Leave"), backFunc);
	...
	addButton(14, (fromMasturbate ? "Back" : "Leave"), backFunc);
}

window.mindwashMeShipVers = mindwashMeShipVers;

// [Me]
function useShipMindwashMeee()
{
	clearOutput();
	showBust("MINDWASH");
	showName("Mindwash Visor");
	output(blockHeader("Mindwashing..."));
	author("Adjatha");
	//first time
	...
	function mindwashVisorScenes(choice = "menu")
	{
		let characterSelect = true;
...
window.useShipMindwashMeee = useShipMindwashMeee;

function mindwashSceneList(sourceType = "", choices = null)
{
	let sceneList = [];
...
function pumpTheSpy()
{
	...
	clearBust();
	clearName();
	output(blockHeader("Pumping the Spy"));
	author("Adjatha");
	...
	output("\n\nWith a grin of wicked satisfaction, you throw off the clasps of your own uniform, revealing the charcoal hued sheath and heavy, basketball-sized testes swinging low against your knees. <i>“No one steals from Thoroughbred and gets away with it, Agent Goblin. It’s time you learned your place.”</i>");
...
function pumpTheSpyII()
{
	clearOutput();
	output(blockHeader("Pump the Spy"));
	author("Adjatha");
...
function pumpTheSpyIII()
{
	...
	output("\n\nHer narrow waist bloats, stomach swelling up with a pregnant bump that swiftly balloons into a nine-month burden. With her womb full to bursting, the gooey consistency of her body starts finding new places to redirect the weight of your subsequent loads. Your cream flows through her, filling her pathetically flat chest, expanding meager A-cups to a proper whore’s double Ds. Her ass inflates to cushiony orbs that practically beg for pounding. Even her hips and thighs fill out, growing wider and fatter until she looks every bit the broodmother you promised her.");
...
function pumpTheSpyIV(cumNum)
{
	...
	else if (!window.checkToyDrawer(BubbleBuddy) || cumNum === 0)
	{
...
//2 Cow-ified
function cowifiedMindwash()
{
	clearOutput();
	clearBust();
	clearName();
	output(blockHeader("Cow-ified"));
	author("Adjatha");
	output("As you walk briskly along the buzzing, neon-lit hall, you wave and flashing your big, bright smile to the clients you pass. A glance to one side catches your reflection in a mirrored meeting room window, so you pause to make a few adjustments. The figure smiling back at you is a young, svelte man, dressed in a sensible but stylish charcoal-gray number. Your suit is fitted exceptionally well and accented with a bold green-silver that picks out your clear, sharp eyes. You give yourself a wink before picking up your earlier, peppy pace.");
...
function cowifiedMindwashV(cumNum)
{
	clearOutput();
	output(blockHeader("Cow-ified"));
	author("Adjatha");
...
function invasionOfTheDickSuckers()
{
	clearOutput();
	clearBust();
	clearName();
	output(blockHeader("Dicksucking Invasion"));
	author("Adjatha");
...
function invasionOfTheDickSuckersII()
{
	clearOutput();
	output(blockHeader("Dicksucking Invasion"));
	author("Adjatha");
...
function invasionOfTheDickSuckersIII()
{
	clearOutput();
	output(blockHeader("Dicksucking Invasion"));
	author("Adjatha");
...
function invasionOfTheDickSuckersIV()
{
	clearOutput();
	output(blockHeader("Dicksucking Invasion"));
	author("Adjatha");
...
function invasionOfTheDickSuckersV(cumNum)
{
	clearOutput();
	output(blockHeader("Dicksucking Invasion"));
	author("Adjatha");
	...
	{
		if (!window.checkToyDrawer(BubbleBuddy) || cumNum <= 0)
		{
			output("\n\nYou glance around and realize your in-simulation orgasms were matched by real world ones as well.");
			if (cumNum <= 0)
			{ /* Nada */
...
function gymTripSceneIV()
{
	...
	output("\n\nThe imposing workout partner’s twin tails snake their way up to wrap around your sweat-slick breasts, squeezing nearly as hard as your thighs on the colorful orb you’ve been bound to. <i>“Hrm? Oh, right! Helping!”</i> He pushes against your back, folding you over while he grabs your ankles and lifts your legs up, leaving you a horizontal plank atop a pliant rubber ball. He tears the hole in your shorts a bit more and slaps his feline cock between your ass cheeks. Rocking you back and forth on the ball, the kaithrit coats his nub-covered member across the slick nectar of your oozing femme cum, polishing your ass at the same time. Then, with his hands firmly around your ankles, he pulls you back, onto his manhood.");
...
function doorToDoorDildos()
{
	clearOutput();
	showBust("MIDEE");
	showName("Mi’Dee");
	output(blockHeader("Door to Door Dildos"));
...
function doorToDoorDildosIV()
{
	clearOutput();
	clearBust();
	clearName();
	output(blockHeader("It’s Over"));
	author("Adjatha");
...
function poorLittleWhiteGel()
{
	clearOutput();
	clearBust();
	clearName();
	output(blockHeader("Record Seeker"));
	author("Adjatha");
...
function poorLittleWhiteGelIII()
{
	clearOutput();
	output(blockHeader("Record Seeker"));
	author("Adjatha");
...
function poorLittleWhiteGelV(cumNum)
{
	clearOutput();
	output(blockHeader("Record Seeker"));
	author("Adjatha");
...
//#2 - Bodysuit Bubbles
function bodysuitBubblesScene()
{
	clearOutput();
	clearBust();
	clearName();
	output(blockHeader("Bodysuit Bubbles"));
	author("Adjatha");
...
function bodysuitBubblesSceneII()
{
	clearOutput();
	output(blockHeader("Bodysuit Bubbles"));
	author("Adjatha");
...
function bodysuitBubblesSceneIII()
{
	clearOutput();
	output(blockHeader("Bodysuit Bubbles"));
	author("Adjatha");
...
function bodysuitBubblesSceneIV()
{
	clearOutput();
	output(blockHeader("Bodysuit Bubbles"));
	author("Adjatha");
...
function bodysuitBubblesSceneV(cumNum)
{
	clearOutput();
	output(blockHeader("Bodysuit Bubbles"));
	author("Adjatha");
...
{
	clearOutput();
	output(blockHeader("Breakin’ the Law"));
	author("Adjatha");
...
function breakingTheLawVI(cumNum)
{
	clearOutput();
	output(blockHeader("Breakin’ the Law"));
	author("Adjatha");
...
function mindwashCelise()
{
	...
	if (flags["CELISE_MINDWASHED"] === undefined)
	{
		...
		processTime(4);
		clearMenu();
		addButton(0, "Next", mindwashCeliseFirstTimeII);
	}
	else
	{
		...
		celiseScene = rand(4);
		if (isSillyModeEnabled() && rand(2) === 0) output("affects a falsetto, proclaiming <i>“Oh, what a lovely tea party!”</i>");
...
function mindwashCeliseFirstTimeII()
{
	clearOutput();
	celise.showAll();
	author("Adjatha");
	...
	pc.changeLust(20);
	clearMenu();
	addButton(0, "Next", mindwashCeliseFirstTimeIII);
}

function mindwashCeliseFirstTimeIII()
{
	clearOutput();
	celise.showAll();
	author("Adjatha");
...
//Imp Mob Ambush
function impMobAmbush()
{
	clearOutput();
	showBust("");
	showName("Imps");
	output(blockHeader("Imp Ambush"));
	author("?? CoC ??");
...
function akbalScene()
{
	...
	showBust("");
	showName("Akbal");
	output(blockHeader("Akbal: Panther God"));
	author("?? CoC ??");
...
function kitsuneSex()
{
	...
	showBust("");
	showName("Kitsune");
	output(blockHeader("Defeated Kitsune!"));
	author("?? CoC ??");
...
//Gold
function goldKitsune()
{
	clearOutput();
	showBust("");
	showName("Gold Kitsune");
	author("?? CoC ??");
...
//Red - teased loss
function redKitsuneLoss()
{
	clearOutput();
	showBust("");
	showName("Red Kitsune");
	author("?? CoC ??");
...
//Follow to Lair
function followingKitsunes()
{
	clearOutput();
	clearBust();
	clearName();
	output(blockHeader("Lost"));
	author("?? CoC ??");
	output("As you explore deeper into the dense wood, you are abruptly aware that your surroundings have grown darker without warning. The back of your neck tingles lightly with a sense of foreboding, and you instinctively ready your fists, feeling uneasy. Wracked with paranoia, you find yourself swiveling to face toward every random noise, and you could swear you just heard a voice through the trees. There it was again! As the ghostly, feminine laughter fills your ears, you are positive that it can’t just be your imagination. You turn left, then right, trying to pinpoint its source, but it truly sounds as though it is all around you now.");
	output("\n\nCatching a glimpse of motion out of the corner of your eye, you whip around to face it, but are surprised to see that the only thing there is a small, pale blue flame, flitting about idly. It dances around hypnotically, and as you stare into its ghostly light, you find your conscious mind growing hazy. Your concerns suddenly seem trivial, and you find yourself relaxing gradually as the ethereal wisp glides along your arms, leaving behind a cool tingle where it touches you.");
	output("\n\nIt seems to be beckoning you to follow it.");
...
function sanuraFucksPC()
{
	...
	output("\n\nTilting your head to the side, you try to look up above, but all you can see past her bristling belly-fur are the pert swells of her cleavage and a few swirling, ephemeral hexagons floating around her wrists while her fingers swivel with inhuman dexterity. There’s a barely audible ‘fwoomp’ followed by a tingling against your loose backdoor, abruptly carrying its magical effervescence into your sphincter as something flat and blunted butts up against it. Your eyes open wide in surprise at the pressure on your backdoor, something Sanura takes absolute delight in seeing, bending her human-half over to take in your expression while her forepaws push you harder onto her magical spear.");
...
function fuckingSphynx()
{
	clearOutput();
	showBust("");
	showName("Sphynx");
	output(blockHeader("Fucking Sphinx!"));
	author("?? CoC ??");
...
//Cum Witch bad end
function cumWitchBadEnd()
{
	clearOutput();
	showBust("");
	showName("Cum Witch");
	output(blockHeader("...Cum Witch?"));
	author("Fenoxo");
...
function cumWitchBadEndII(cumNum)
{
	clearOutput();
	showBust("");
	showName("Cum Witch");
...
function yvonneFuckies()
{
	...
	output("\n\nA shiver runs through the dog-morph’s entire body, culminating in a cock-wringing contraction that ripples through her cunt, milking you with her slippery twat. It works, and you lean over her prostrate form as you bottom out inside her, her sweat-matted fur smearing your face with her scent as you cum. Your jizz bubbles out to fill her depths, the spunk surging through her as it moves towards her womb. Yvone sighs dreamily, <i>“Ahhhh...”</i> while you empty your quartet of basketball-sized balls inside her unprotected womanhood. With a thoroughly cream-stuffed twat beneath you, you pull out, accompanied by a exhalation of female pleasure.");
...
function barkOnceForCeraph()
{
	...
	output("\n\nSWAT! Her hand claps against a cheek, lingering to squeeze the raw flesh after the spank. She explains, <i>“That was for resisting, and this...”</i> You feel a slick, wet warmth at your tender, rear opening. <i>“...is for being my obedient, playful little doggie.”</i> Her hips press the hot spear harder against your backdoor. The cunt-slick tool slides right on in thanks to your capacitive anus, the gentle nubs each massaging your snug ring as they slide on through. You groan as you’re taken by your owner, catching yourself halfway and arresting the pleasured grunt as soon as you can, but it doesn’t matter - the damage is already done.");
...
function loppeTheRabbot()
{
	...
	output("\n\nLoppe looks at you, studying you to see if you’re mocking her. But when she detects only honesty, she sighs and breaths a sigh of relief. <i>“Sugar, you really are sweet. I can’t tell you the number of times I’ve been rejected just because I’m a [herm|npc|f]... anyways, we can chat later. Our food is here.”</i> She points toward the waitress holding your orders in a tray. You nod to her in agreement and turn your attention towards the food, ready to savor the sweets.");
...
function loppeBJ()
{
	...
	output("\n\nUsing her hands and arms, Loppe grinds her soft boobs against your wide, blighted cock, milking a few more dollops of cum from you, slickening her breasts, as well as your saliva-coated prick. You shudder and instinctively thrust your cock into her soft cleavage; her skin against your corrupted cock sends tingles of pleasure rippling through your loins. The half-breed doesn’t stop her ministrations, barely noticing your reaction. All she does is giggle as you continue pumping more of your slick pre all over her breasts. Murmuring and moaning appreciatively, you continue to thrust and grind, eagerly spilling pre-cum all over your [herm|npc|f] lover’s cleavage and midriff, though you can’t prevent one sudden spurt from landing right on her face.");
...
function loppeAnal()
{
	...
	output("\n\nYou nod your head, still silent, letting her take her cue from your actions. <i>“Okay then, let’s get you prepped up first.”</i> Loppe insists, moving to her closet. She opens it and looks around. <i>“Hmm... aha!”</i> Your lover triumphantly lifts a rather large bottle containing a strange bluish liquid, the label sporting a design featuring an extremely well-hung, happy looking [herm|npc|f] with an erection. <i>“Good old lubrication!”</i>");
...
function heckelScene()
{
	...
	addButton(2, "Top The Dog", heckelTopDog);
}

function heckelGetsSucked()
{
	clearOutput();
	showBust("");
	showName("Heckel");
...
function heckelTopDog()
{
	...
	output("\n\nOf course, with Heckel around you knew you’d be risking your ass, but at least now you’re going to get a shot at hers. You nod at the surprised hyena [herm|npc|f] in acceptance of her wager and suggest the most obvious way to settle just who runs this gym - an arm wrestling contest.");
	...
	output("\n\nYou casually rise and circle round the table. Before she can react, you dexterously slip your hands down to Heckel’s sculpted, furry backside and lift her high, easily flipping her up onto the wooden surface. The muscle-[herm|npc|f] grunts softly as she comes to rest upon the polished wood, her flopping dog-cock already starting to thicken with the telltale engorgement of oncoming arousal. Smirking, you gently prod at the growing knot and tease her - she might be an avowed dom, but her dick seems excited by the prospect of sex on any terms.");
	...
	output("\n\nOnly once you have fully sated yourself upon the still-climaxing [herm|npc|f]’s vagina do you pull out. Her gash gapes in the shape of your maleness as you exit, dribbling sperm and continuing to convulse anew, milking the phantom member she wishes were still inside her. You plop your distended, bumpy demon-dick into her mouth, and she licks it without thought or question, cleaning the mixed love-juices from you until you shine with her glossy spit.");
...
function roxanneDrinkingScene()
{
	...
	output("\n\nGiggling and nearly tripping up on your own slimy cilia, you stumble up to the corrupted deer-taur. He looks at your wobbling stance, nearly-vacant eyes, and dopey grin before he shakes his head from side to side and says, <i>“No.”</i> Yessss! You finally got so drunk that Roxanne has no excuse not to pack your drunk ass full of lizan-spoo! A scaled hand slaps your expansive, squeezable ass spinning you around to fall drunkenly into the pirate’s soft, cushy chest. <i>“Don’t worry, I’ll be gentle,”</i> she whispers, hooking an arm around your sagging frame.");
	...
	{
		let hasCock = (rand(2) === 0);
		let hasVagina = (rand(2) === 0);

		output("\n\nGods, your head is swimming! The room is pitching from side to side, and you giggle as you idly wonder if this is what it would be like aboard Roxanne’s ship. Still, the well-endowed [shemale|npc|f] doesn’t seem to mind your tipsy bumbling in the slightest. She hooks her hands under your soft, padded backside and squeezes the spacious ass-flesh appreciatively, then growls, <i>“Such a nice, round ass; perfect for stuffing with cock! You’re wasted as a champion. You’d make a better cabin-girl,”</i> into your ear as she drags you towards her wagon.");
		...
		output("”</i>");
		output("\n\nYou whimper as Roxanne thrusts herself further forward, burying another few inches of potent lizard-cock inside you. Its sheer size shifts your innards slightly, and you’re sure there must be a visible bulge on your tummy by now. The [shemale|npc|f] sighs blissfully and continues to work her dick further and further inside you. Finally, when you feel you can fit no more, her large, quaking testes press against your rump. If you didn’t have such a round ass, she’d probably still be trying to go further inside, but your plump derriere actually helps to distance her from your vulnerable pucker - by a bit.");
...
function smallPoisontailEppie(cumNum)
{
	clearOutput();
	showBust("");
	showName("Roxanne");
...
function largePoisontailEppie(cumNum)
{
	clearOutput();
	showBust("");
	showName("Roxanne");
...
//Drider Table-turner
function driderTableTurner()
{
	clearOutput();
	showBust("");
	showName("Drider");
	output(blockHeader("Drider!"));
	author("?? CoC ??");
...
function driderTimesEpilogue(cumNum)
{
	clearOutput();
	showBust("");
	showName("Drider");
...
function driderTimesEpilogue2(cumNum)
{
	clearOutput();
	showBust("");
	showName("Drider");
	...
	author("?? CoC ??");
	output("Even later...");
	output("\n\nYou collapse into the dirt, not even caring that it stinks of dried cum and girl-jizz. You’ve completely emptied out every speck of sperm from your quartet of basketball-sized balls, and you even kept going for a while after that. It wasn’t until one of your bumpy demon-dicks went soft at last that you found enough peace to lie down. The longer you sit there, the more your strength returns to you, and the more the submissiveness placed into you by her venom recedes. Still, you’ve tired yourself out too much to leave at the moment. You shiver and wrap your arms around her, snuggling against her while a primal, buried part of your brain insists that she’s YOURS. You fall asleep like that, resting dreamlessly even through your mate’s waking and departure.");
	processTime(40);
	cumNum += pc.cumQ();
	pc.orgasm();
	clearMenu();
...
//Minotaur Quest
//Gloryholes
function gloryholesScene()
{
	clearOutput();
	showBust("");
	showName("Gloryholes");
	output(blockHeader("Gloryholes!"));
	author("?? CoC ??");
...
function doggieGHole()
{
	...
	output("\n\nYou throw yourself onto the swollen dog-cock and easily engulf the whole thing, spreading your lips wide around the thick knot. You can feel it bouncing the uvula in your throat around but you manage to suppress your gag reflex like a good slut while you work the sweaty dick, giving it a thorough tongue bath. You wrap your lips around the base of the knot, feeling it start to stretch your jaws apart. You pull back before it can lock you like that, and squeeze the knot in your hands. It cums, unloading thick wads of gooey cum into your mouth.");
...
function minotaurAddictGHole()
{
	clearOutput();
	showBust("");
	showName("Minotaur");
	output(blockHeader("Minotaur Cock"));
	author("Fenoxo");
...
function minotaurXCowgirls()
{
	clearOutput();
	showBust("");
	showName("Minotaur & Cowgirl");
	author("?? CoC ??");
...
function minotaurCampBadEnd()
{
	clearOutput();
	output(blockHeader("Minotaur Camp"));
	author("?? CoC ??");
...
function izmaLoss()
{
	...
	if (rand(2) === 0)
	{
		...
		output("\n\nAfter a lengthy fuck, you grunt loudly as your knee-length, nubby shaft swells, blasting streamers of jizz into Izma’s womb and onto her groin, causing Izma to cry out loudly. You sigh happily and push back from her, weakly getting to your slimy cilia. You’re not done yet though, not fully.");
...
function CoCSceneEpilogue(cumNum)
{
	clearOutput();
	output(blockHeader("Mindwashed"));
	...
	{
		if (!window.checkToyDrawer(BubbleBuddy))
		{
			output("\n\nYou glance around and realize your in-simulation orgasms were matched by real world ones as well.");
			if (cumNum <= 0)
			{ /* Nada */
...
function jezebelMindwash()
{
	clearOutput();
	output(blockHeader("Jezebel’s Dungeon"));
	author("Carol J");
	let cumNum = 0;
...
function jezebelMindwashII(cumNum)
{
	clearOutput();
	output(blockHeader("Jezebel’s Dungeon"));
	author("Carol J");
...
function jezebelMindwashIII(cumNum)
{
	clearOutput();
	output(blockHeader("Jezebel’s Dungeon"));
	author("Carol J");
...
function jezebelMindwashIV(cumNum)
{
	clearOutput();
	output(blockHeader("Jezebel’s Dungeon"));
	author("Carol J");
...
function jezebelMindwashV(cumNum)
{
	clearOutput();
	output(blockHeader("Jezebel’s Dungeon"));
	author("Carol J");
...
// (scene: First Time 2)
function shekkaFirstTimeMindwashII()
{
	clearOutput();
	shekka.showAll();
	author("B!");
...
// (scene: First Time 3)
function shekkaMindwashFirstTime3(touch)
{
	clearOutput();
	shekka.showAll();
	author("B!");
	...
	//aphro venom!
	if (pc.hasTongueFlag(GLOBAL.FLAG_APHRODISIAC_LACED) && shekka.spitAddicted()) shekka.spitAddiction(4);
...
function shekkaFirstTimeMindwashIII()
{
	clearOutput();
	shekka.showAll();
	author("B!");
...
// (scene: Random 1 Cont.)
function shekkaRepeatMindwashVarI(touch)
{
	clearOutput();
	shekka.showAll(true);
	author("B!");
...
function shekkaRepeatMindwashVarIOutro()
{
	clearOutput();
	shekka.showAll(true);
	author("B!");
