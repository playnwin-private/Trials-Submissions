...
import { RoomFlags } from "engine/globals";
import { ShockHopper } from "characters/ShockHopper";
import { JumperJumpsuit } from "items/Armor/JumperJumpsuit";
import { JumperJumpsuitSlutty } from "items/Armor/JumperJumpsuitSlutty";
import { RattyArmor } from "items/Armor/RattyArmor";
import { AugmentWeaveArmor } from "items/Armor/AugmentWeaveArmor";
import { StarViperSlutwear } from "items/Armor/StarViperSlutwear";
import { MaikesCollar } from "items/Accessories/MaikesCollar";
import { CodexManager } from "engine/CodexManager";
import { StringUtil } from "engine/StringUtil";
import { Achievements } from "engine/Achievements";

function shockHopperBustString(nude = false, gun = false)
{
	return `SHOCK_HOPPER${nude ? "_NUDE" : (gun ? "" : "_NO_GUN")}`;
}

function LDCBustString()
{
	return `LDC`;
}

/*
function showShockHopper(nude = false, gun = false)
{
	showBust(shockHopperBustString(nude, gun));
	showName("Shock Hopper");
}
...
function showShockHopperAndLDC(hopperNude = false, hopperGun = false)
{
	showBust(shockHopperBustString(hopperNude, hopperGun), LDCBustString());
	showName("Hopper & L.D.C.");
}

function showLDCAndShockHopper(hopperNude = false, hopperGun = false)
{
	showBust(LDCBustString(), shockHopperBustString(hopperNude, hopperGun));
	showName("L.D.C. & Hopper");
}
...
function shockHopperEncounter()
{
	if (flags["SHOCK_HOPPER_DEFEATED"] === undefined)
	{
		...
		if (pc.hasSlaveOutfit()) output(" I didn’t order more skanks... did I?");
		//pcLapineEarOrJumperUniform:
		else if (pc.earType === GLOBAL.TYPE_LAPINE || pc.inventory.armor instanceof JumperJumpsuit || pc.inventory.armor instanceof JumperJumpsuitSlutty) output(" I didn’t call a meeting and I know I didn’t ask for one of you sweaty sluts.");
		//pcMouseEarsOrRatArmor:
		else if (pc.earType === GLOBAL.TYPE_MOUSE || pc.inventory.armor instanceof RattyArmor)
		{
		...
		//pcSecOpWeaveArmor: 
		else if (pc.inventory.armor instanceof AugmentWeaveArmor) output(" W-what are one of you technophiles doing here... Wait, you don’t have an antenna!");
		//pcViperSlutwear: 
		else if (pc.inventory.armor instanceof StarViperSlutwear) output(" Oh crap, how did you- wait, you’re not a Viper, and only those snake bitches wear that.");
		...
		//pcLapineEarOrJumperUniform: 
		if (pc.earType === GLOBAL.TYPE_LAPINE || pc.inventory.armor instanceof JumperJumpsuit || pc.inventory.armor instanceof JumperJumpsuitSlutty) output("\n\n<i>“[pc.HeShe] isn’t one of mine!”</i>");
...
window.shockHopperEncounter = shockHopperEncounter;
...
function loseToShockHopper()
{
	author("William");
	showShockHopperAndLDC(true);
	output(blockHeader("Defeat"));
	let bustedCount = enemy.statusEffectv1("Busted");
	if (pc.HP() <= 1)
	{
	...
	if (pc.isDog()) output("pretty puppy");
	else if (pc.isCat()) output("nice kitty");
	//pcGryvainOrScaled:
	else if (pc.hasScales() || pc.score("gryvain").score >= 6) output("shiny prize");
	//pcLaquineRabbit:
	else if (pc.score("laquine").score >= 5) output("new girl");
	//pcMouseRodenian:
	else if (pc.score("rodent").score >= 4) output((pc.tallness < 60 ? "cute runt" : "cute rat"));
	//pcLeithanOrHorse:
	else if (pc.isTaur() || pc.score("horse").score >= 4) output("new pony");
	...
	let collared = pc.inventory.accessory instanceof MaikesCollar;
	...
	else
	{
		...
		if (pc.isBimbo() || pc.isBro())
		{
			...
			if (pc.isHerm()) output("your [hermhood|pc]?");
		...
		if (pc.hasGenitals())
		{
			output(", particularly around your " + (pc.hasCock() ? "[pc.cocksLight]" : "[pc.clits]") + ", exploring your strangely distant " + (!pc.hasCock() ? "feminine " : "") + "erection" + (pc.cockTotal() > 1 || (!pc.hasCock() && pc.totalClits()) ? "s" : ""));
		}
		...
		else if (pc.hasVagina()) output("parted honeypot" + (pc.vaginaTotal() > 1 ? "s" : ""));
		...
		output(" adoringly, <i>“and then you’ll learn why the Jumpers " + (isSillyModeEnabled() ? "rule" : "are on top") + ".”</i>");
		processTime(25);
		pc.changeLust(50);
		clearMenu();
...
function loseToJumpBossPart2Busted()
{
	clearOutput();
	author("William");
	showShockHopperAndLDC(true);
	...
	if (pc.hasCock())
	{
		...
		if (pc.hasVagina()) output("Your [pc.pussies], too, " + (pc.vaginaTotal() === 1 ? "has" : "have") + " become so numb and drippy that the clamps feel like natural distensions. ");
		...
		// Merge
		output("Your bunny-queen rocks back and forth, licking from " + (pc.balls > 0 ? "your sack" : "[pc.base]") + " to your crown, pressing your drooling member" + (pc.cockTotal() > 1 ? "s" : "") + " to your [pc.belly] and then to her bosom." + (pc.balls > 0 ? " Often times her nipples poke and prod against your [pc.balls], almost breaking your concentration." : ""));
...
function moreHopperBadEndStuff()
{
	clearOutput();
	author("William");
	showShockHopperAndLDC(true);
	...
	if (pc.hasCock()) output(" and your twitching tool" + (pc.cockTotal() > 1 ? "s" : ""));
...
function hopperLossBadEndCombinedNewLifeShit()
{
	clearOutput();
	author("William");
	showShockHopperAndLDC(true);
...
function loseToJumpBossPart2Nonbust()
{
	...
	else if (pc.hasVagina()) output("achingly empty cunt" + (pc.vaginaTotal() > 1 ? "s" : ""));
	...
	else if (!pc.hasSnakeByte()) output(", wanting to show her how pluggable your throat is");
	...
	output("\n\nThe warm pressure on your " + (pc.hasSnakeByte() ? "erogenous " : "") + "throat-muscles would make you cum on the spot, make you whine and perhaps choke, but after another barely noticed thickening to the electro-blanket those impulses subtly fade. ");
	...
	else output("A high-pitched whine slips through every chance you get to breathe, imploring her to inflate you and pump as deep as your biologies allow. This bunny’s boner was <i>made</i> for your throat; that taut, swollen slut-shaft is the foot in your sock. Your walls hug her tender stiffness better than a pussy!" + (pc.hasSnakeByte() ? " A claim you can back up, the way your ribbed internals methodically work that spit-greased pole!" : ""));
	...
	output("\n\nYou try to open your eyes, only to feel the sting of cascading [herm|npc|f]-goo webbing them shut. What tiny glimpses you caught were of her and her pulsating shaft close to your lips. There’s nothing else it could have been. You’re not sure anything can be trusted right now except the narrowing of your vision and the abating of consciousness. Suddenly, you feel hollow inside.");
	output("\n\nAnd then, you let go.");
	//[Next]
	processTime(30);
	pc.orgasm();
...
//PC Victory
function victoryOverShockyHoppy()
{
	...
	clearMenu();
	addButton(0, "Take It", takeTheAccessCodesAndProbes, "Take It", "Just take the access codes and leave the bunnies alone.");
	addButton(1, "Save Them", saveThemSluts, "Save Them", "Oh no, you’re not letting him walk off with all these innocent women.");

	if (pc.cockThatFits(enemy.analCapacity()) >= 0 || pc.hasHardLightEquipped())
	{
		addButton(2, "Dom His Butt", () => penisRouter(domTheLDCsAss, enemy.analCapacity(), true, 0), "Dom His Butt", "Give the bunny-boy a taste of his own medicine before anything else.");
	}
	else
	{
		addDisabledButton(2, "Dom His Butt", "Dom His Butt", "You need a penis that fits inside him or a hardlight strap-on for this.");
	}
	if (pc.cockThatFits(enemy.vaginalCapacity(0)) >= 0 || pc.hasHardLightEquipped())
	{
		addButton(3, "Fuck Hopper", () => penisRouter(fuckTheHopper, enemy.vaginalCapacity(0), true, 0), "Fuck Hopper", "Bend the blue dickbunny over and give her a good fuckin’ as punishment.");
	}
	else
	{
		addDisabledButton(3, "Fuck Hopper", "Fuck Hopper", "You need a penis that fits inside her or a hardlight strap-on for this.");
	}
	...
	else if (pc.cockThatFits(enemy.analCapacity()) >= 0) addButton(5, "Threesome", () => penisRouter(laquineShockyThreesomes, enemy.analCapacity(), true, 0), "Threesome", "If all these depraved laquines do is sit around getting laid, they wouldn’t mind one last round together. One you’ll be leading, whether L.D.C. likes it or not.");
	...
	if (pc.hasCock()) addButton(7, "Get Service(P)", () => getWorshippedByBuns(true), "Get Service(P)", "Those two bunnies are the ones <i>mostly</i> in charge of Zhèng Shi. Get ‘em on their knees and servicing you.");
	else addDisabledButton(7, "Get Service(P)", "Get Service(P)", "You can’t receive real oral service without a dick!");
	if (pc.hasVagina()) addButton(8, "Get Service(V)", () => getWorshippedByBuns(false), "Get Service(V)", "Those two bunnies are the ones <i>mostly</i> in charge of Zhèng Shi. Get ‘em on their knees and servicing you.");
...
function takeTheAccessCodesAndProbes()
{
	clearOutput();
	showShockHopperAndLDC();
	output(blockHeader("Victory!"));
	...
	addButton(1, "Leave Them", leaveThemSluts, "Leave Them", "Fuck ‘em, you just need the probe.");
}

//Leave Them
//Fuck ‘em, you just need the probe.
function leaveThemSluts()
{
	clearOutput();
	showShockHopperAndLDC();
	output(blockHeader("Victory!"));
...
function rideHopperSlut(x)
{
	...
	//need to deal with vaginarouter returning -1 if nothing fits now
	if (x < 0 && pc.hasVagina()) x = pc.defaultVaginaIndex;
	...
	pc.changeLust(50);
	clearMenu();
	addButton(0, "Next", rideHopperSlut2);

	function rideHopperSlut2()
	{
		clearOutput();
		showShockHopperAndLDC(true);
		output(blockHeader("Victory!"));
...
function domTheLDCsAss(x)
{
	...
	output("\n\n<i>“Woah, hey uh, what are you doing? T-there’s no need for, I mean there’s plenty of others here! Take your pick!”</i> Frantic words tumble out of his mouth; the " + (enemy.HP() <= 1 ? "beaten" : "debilitatingly aroused") + " " + (isSillyModeEnabled() ? "energizer" : "shock") + " bunny whimpers and lurches in your direction.");
	// PC not taur
	if (!pc.isTaur())
	{
		...
		output(" when his breath catches, when he realizes how much control <i>you</i> now have. You take one of Zhèng Shi’s captains by his tiny shoulders and flip him around, dragging him closer to your lap until he’s propped " + (pc.legCount === 2 && pc.hasKnees() ? "over your knee" : "up on all fours") + ".");
...
function domTheLDCsAss2(x)
{
	...
	output("\n\nYou can’t stifle your laughter anymore, running a hand " + (!pc.hasHair() ? "over your sweaty head" : "through your sweaty [pc.hairsNoun]") + ", finding new ways to pound him, new ways to bump and grind through his contracting interior" + (pc.hasTailCock() || pc.hasTailCunt() ? " while the Hopper moans slack-jawed to your rear" : "") + ". You plant a [pc.hand] on his head and pull back, straining his neck and adding to the stimulation of his reaming. Longer and longer ropes of laquine nut surge to the front of the bed");
	...
	output(". That small orgasm triggers the real one" + (pc.hasTailCunt() || pc.hasTailCock() ? ", spurred on by the hypnotic tensions rhythmically undulating through your extra genitalia from the now-detonating Hopper" : "") + ".");
	...
	if (x >= 0)
	{
		...
		if (cumQ < 150)
		{
			...
			output("\n\nThe dribbles and terran-typical ribbons of [pc.cumVisc] [pc.cumNoun] you fill his ass with are nothing compared to the turgid lines of bunny nut " + (isSillyModeEnabled() ? "cheerios" : "cream") + " splashing against the wall, his chin, and whatever the hell else is in the way until he’s practically swimming in his own feracity. L.D.C.’s climax does eke out the most of your load, however, and you’re sure the warmth of your [pc.cumColor] seed is to his liking" + (pc.cockTotal() > 1 ? ", particularly those lines you’ve drawn across his pre-soaked ass" : "") + ".");
		}
		// PC plenty of cum
		else if (cumQ < 4000)
		{
	...
	else
	{
		...
		if (pc.hasVagina())
		{
			output("Your cunt" + (pc.vaginaTotal() > 1 ? "s grind" : " grinds") + " so tightly around something not there that you’re able to feel the caress of muscle against white-hot vaginal muscle as [pc.girlCum] ");
			if (pc.wettestVaginalWetness() < 3) output("dribbles from your deliciously glazed slit" + (pc.vaginaTotal() > 1 ? "s" : ""));
			else if (!pc.isSquirter()) output("runs in thick streams from your beautifully drenched puss" + (pc.vaginaTotal() === 1 ? "y" : "ies"));
...
//[Get Worshiped]
// Tooltip: Those two bunnies are the ones <i>mostly</i> in charge of Zhèng Shi. Get ‘em on their knees and servicing you.
...
// William
function getWorshippedByBuns(cockUse)
{
	clearOutput();
	showShockHopperAndLDC(true);
	output(blockHeader("Victory!"));
	...
	output(". Your glistening endowment" + (pc.cockTotal() + pc.vaginaTotal() > 1 ? "s are" : " is") + " attractive enough to earn a few cute ‘oohs’ from the naked audience, though your choice of partners fidget.");
	...
	if (!pc.isTaur())
	{
		...
		else output("juice-drooling slit" + (pc.vaginaTotal() > 1 ? "s" : ""));
	...
	else
	{
		...
		else
		{
			...
			else output(" You may be as loose as a " + (pc.score("laquine").score >= 5 ? "broodmother" : "laquine") + ", but with my size that won’t matter!");
	...
	if (!pc.isTaur() && cockUse)
	{
		...
		else output("Tears well at the corners of her glazing eyes trying to fit even a quarter of your " + (isSillyModeEnabled() ? "pounder" : "[pc.cockNoun " + x + "]") + " in");
		...
		output("\n\nAs pleasing as this scenario is, you’re not totally blind to what’s going on below. The shock-bun’s sizeable slab of [herm|npc|f]-cock is nothing compared to the elephantine mast between her lover-boys legs. Every time that lurid tower of laquine meat pulses it grows just a bit bigger, snaking up at an angle that’ll soon overshadow him and be pressing at your own. It’s soon pressed down and aimed between your legs, though its defiant throbs bounce it skyward.");
		...
		output("\n\nIt’s not just the feel of that thick muscle that has you clenching your [pc.thighs] together, it’s that you can feel his consternation behind it - that little bit of resistance tickles your dominant side, and it makes his hand-smothering balls lurch closer to orgasm. The " + (isSillyModeEnabled() ? "ink-ribbun" : "ink-bun") + " wants to mouth off and act tough, but he also can’t admit he fucking loves being your bitch. You can see that subby side no clearer than when you grab him by his tall ears and rein him in.");
		output("\n\nYou guide the imperious pirate down your girth until he’s kissing her right around your swollen tip");
		if (pc.cockTotal() > 1) output(", the other slaves rubbing your extra dick" + (pc.cockTotal() > 2 ? "s" : "") + " into their smooth foreheads, making sure these oversexed rabbits are thoroughly lathered in your cascading " + (pc.isHerm() ? "[herm|pc]-" : "") + "goo");
		output(".");
		...
		output("\n\nThe blue-bun’s eyes cross and seal shut. Her whole body squirms and humps in the throes of orgasm - the [herm|npc|f]-boss’s cock has taken full control of her brain. A powerful gout of cum squirts into the air and rains down on a few slaves. That brace of girl-dick is aimed away from you by the slaves who ease out the modded-up loads churning out of her scent-billowing maleness in long gouts of familiar stickiness.");
		...
		if (!pc.hasTailCunt()) output((pc.legCount > 1 ? "between your [pc.legs]" : "down") + ", splattering the kitty-girl behind you and painting a wide cone of the room white with laquine virility");
	...
	else if (!cockUse && !pc.isTaur())
	{
		...
		if (pc.vaginaTotal() > 1) output(", they’re even slipping their thumbs and fingers into the other juicy entrance" + (pc.vaginaTotal() > 2 ? "s" : "") + " you have");
		output(".");
		...
		output("\n\nThe dusky laquine’s muzzle smashes into your nether regions until his cheeks are scissored between the supple fat of your [pc.thighs]. The harder his tail wags, the more inches get added to his mammoth horse cock! <i>“Good boy,”</i> you pet the horned up pirate behind the floppy ears, fingers creasing the soft surface of his head, <i>“you’re gonna be there awhile. Get cozy,”</i> you titter, rocking him back and forth across your ambrosial slit" + (pc.vaginaTotal() > 1 ? "s" : "") + ", ready to fuck his muzzle properly.");
		...
		output("\n\nWith a pitiful whine, the cream-colored XO gazes up, nuzzling her head towards your [pc.pussies]. " + (enemy.HP() > 1 ? "She unzips her orange suit" : "She pulls the remnants of her suit off") + ", baring her beautiful, creamy breast to you. She suckles at " + (pc.totalClits() === 1 ? "your [pc.clit]" : "one clit, then the other") + ", unable to get her lips around " + (pc.vaginaTotal() > 1 ? "any of " : "") + "your dripping " + (pc.vaginaTotal() === 1 ? "femininity" : "femininities") + "." + (pc.clitLength >= 4.5 ? " You consider forcing her down your clitoral fuck rod, but she seems so desperate!" : "") + " Their cocks are so turgid that two slaves who’ve snaked behind them are jacking those prideful boners with casual ambivalence, keeping them aimed down." + (enemy.lust() >= enemy.lustMax() ? " However, the azure slut can’t keep it in her gurgling balls, and musky laquine cum shoots weakly or dribbles out of her cock. It’s only a sign of things to come." : ""));
		...
		if (pc.balls > 1)
		{
			...
			if (pc.hasCrotchFlag(GLOBAL.FLAG_UNIBALL)) output("taut");
		...
		output(". Your [pc.pussies] rob" + (pc.vaginaTotal() === 1 ? "s" : "") + " your entire body of muscle control, trapping " + (!pc.hasVaginas() ? "its" : "their") + " invaders in your leaky quim" + (pc.vaginaTotal() > 1 ? "s" : "") + " and slathering them with new coats of femslime. The moans you’re loosing turn to fevered wails as sweat trickles down your neck in saccharine detail.");
	...
	else if (cockUse)
	{
		...
		if (pc.hasVaginas())
		{
			...
			if (pc.vaginaTotal() === 2) output(", a hand also moving to attend the other slit");
			else if (pc.vaginaTotal() > 2) output(", those hands moving to attend the other slits");
			output(".");
	...
	else
	{
		...
		if (pc.vaginaTotal() > 1) output(", though " + (pc.vaginaTotal() === 2 ? "one paw" : "his paws") + " slide into your other slit" + (pc.vaginaTotal() > 2 ? "s" : ""));
		output(".");
		...
		output("\n\nStrength waning, you fall forward, most pressure still on your back, sensing that the black-furred dildo and his creamy LT are cumming. Your [pc.pussies] feel" + (pc.vaginaTotal() === 1 ? "s like a collapsing dam" : " like collapsing dams") + ". Gouts of [pc.girlCum] squirt and spray into their cum-soaking fur. Vaginally-slimed pelt frigs against the entrance of your fragrance-spurting cunny. The heat generated by those feverous rubs is nothing short of divine.");
		output("\n\nEyes nearly rolling back, you embed the slut-buns in your nethers, scissoring them between the floor and your [pc.ass]" + (pc.balls > 0 ? ", masking them with your ballmusk too" : "") + ". The jumpers unload onto your underbelly but mostly across their girls" + (pc.hasTailCunt() ? " after your [pc.cuntTail] is literally blown off by the high impact cumsplosion that L.D.C. just smacked it with" : "") + ". The only thing carrying you through your orgasm is the force of lust pounding away at your head with every searing flash of bliss.");
		...
		output("\n\nBy the time it all settles down and you stand again, [pc.hands] pressed to your chest, you breathe. The pain of it all disappears in your next few shudders, regarding the self-appointed lord and lady of Zhèng Shi with a smile. Their coats of fur are wholly matted in your juices and their body odors are going to be smelling of you for the weeks to come. L.D.C. doesn’t seem that upset by it either, gurgling and struggling to rise with a stupid grin writ upon his [pc.girlCumNoun]-splattered muzzle.");
...
// William
function laquineShockyThreesomes(x)
{
	clearOutput();
	showShockHopperAndLDC(true);
	output(blockHeader("Victory!"));
...
function laquineShockyThreesomes2(x)
{
	clearOutput();
	showShockHopperAndLDC(true);
	output(blockHeader("Victory!"));
	...
	output("\n\nHis twin medial rings swell out as thick as a bulbous knot, and are surely capable of leaving indents in whatever hole he manages to wedge that slab of bunny-cock into. When the flat of the blue-bun’s tongue trails around one veiny side, all the spit left in its wake is smeared down that heaven-piercing rod by her diligent hand. " + (pc.isBimbo() ? "Yours too! You’re licking that laquine lollipop with all you’ve got! " : "") + "Her movements demonstrate unerring expertise in ravishing oral service, and you ");
	...
	else if (pc.cockTotal() > 1) output("a slap of your extra dick" + (pc.cockTotal() > 2 ? "s" : "") + " to her rump");
...
function laquineShockyThreesomes3(x)
{
	clearOutput();
	showShockHopperAndLDC(true);
	output(blockHeader("Victory!"));
	...
	output("\n\nYour knees give out, muscle control robbed by your quickening hips. Your [pc.toes] curl into the soft fabrics as beads of spit and visible puffs of heavy air bellow from your mouth. You’re whining and grunting - you’re giving it all you’ve got. " + (!(["laquine", "rabbit", "bunny"].includes(pc.raceShort())) ? " If laquines go hard, then you’re going harder." : " Laquines go hard, but you’re a shining example of their race right now.") + " The lurid ‘schlucks’ of pre, the swelling of two bellies, and the deafening moans of the cumming hopper bring you to that euphoric edge.");
	output("\n\nDominating those two " + (y < 0 ? "in sequence" : "simultaneously") + ", asserting as powerfully as you possibly can on the pirate king and queen of Zhèng Shi... It’s fucking <b>hot</b>. Everything is so hot! You can’t feel anything in your seething loins anymore. Keening cries shake what you can perceive down to all that matters: the two bunny butts you’re stretching into loose permanence.");
...
function drainBunnies(x = 0)
{
	...
	//need to deal with vaginaRouter returning -1 if nothing fits now
	if (x < 0 && pc.hasVagina()) x = pc.defaultVaginaIndex;
	//just in case defaultVaginaIndex is set to -1
	if (x < 0) x = 0;
	...
	if (pc.isAmazon(false))
	{
		...
		output("\n\n<i>“F-fuck you...!”</i> he wheezes, finally surfacing below, the slaves laughing and jeering at the floppy-eared laquine. His cock firmly in hand, you slam down against it, launching a comically thick off-white lasso across the room to the audience’s satisfaction. And your own. " + (isSillyModeEnabled() ? "Home run! " : "") + "You tap the side of LDC’s head, grinning at his enduring erection.");
	...
	else if (pc.isBimbo())
	{
		...
		output("You answer with a very eager nod " + (pc.hasHair() ? "that throws [pc.hairsNoun] around " : "") + "- good [pc.boyGirl]s wait for permission! Sucking dick only when asked to is the best kind of pleasure!");
		...
		output("\n\nYou’re guided upwards to the summit of moistened bunny-cock. So much to suck and so little time! Your [pc.pussies] " + (pc.vaginaTotal() === 1 ? "has" : "have") + " been " + (!pc.isSquirter() ? "streaming" : "flooding") + " with ambrosial [pc.girlCumNoun] for so long the sheets are totally stained [pc.girlCumColor] with dispensed girlmusk!");
...
function drainBunnies2(x)
{
	...
	output("\n\n<i>You’ve claimed him.</i> You’re mounted on the biggest pole on Zhèng Shi, riding it (or at least trying to) and <i>fucking void</i> does it feel <b>awesome!!</b>");
	...
	output("\n\n<i>“It’s coming, you feel that..!? My girls wish they could cum this fuckingggg hhhaaaaardd!!”</i> The bunny-bitch you had to get around just hilted her rod in you, and all the weight of her modded up balls flows through your inhumanly inflated body. You’re more cum than [pc.race]." + (!(["laquine", "rabbit", "bunny"].includes(pc.raceShort())) ? " All of the bunny-batter pumped into you certainly makes you an honorary laquine!" : "") + " Dizziness and ecstasy vie for control of your mental remnants as the combined virility of two unimaginably gifted hyper-bunnies replaces all the blood in your body with cum and more cum.");
...
function postSexShockHopperVictoryEpiloggieDoggie(scene = 0)
{
	clearOutput();
	showLDCAndShockHopper(true);
	author("William");
	...
	processTime(2);
	clearMenu();
	addButton(0, "Free", freeDemHaremSlootipies, "Free", "Free the harem-girls. They’ll be able to find their own way out, hopefully without getting into further trouble.");
	addButton(1, "Don’t Free", dontFreeThemSlootySloots, "Don’t Free", "They’re fine where they are, and you can’t go stirring up more trouble than you already have.");
}

//[Free]
// Tooltip: Free the harem-girls. They’ll be able to find their own way out, hopefully without getting into further trouble.
function freeDemHaremSlootipies()
{
	clearOutput();
	showLDCAndShockHopper(true);
	author("William");
...
function dontFreeThemSlootySloots()
{
	clearOutput();
	showLDCAndShockHopper(true);
	author("William");
...
window.ldcShockHopperPregnancyEnds = function ()
{
	clearOutput();
	clearBust();
	clearName();
	output(blockHeader("Birthing!"));
	author("William");

	let se;
	let numChildren = 0;
	//let bRatingContrib = se.value2;
	//let pregSlot = se.value3;
	//let babym = (se.value4 === 1);
	...
	//let loc = getPlanetName().toLowerCase(); //current planet
	...
	else
	{
		...
		if (isSillyModeEnabled()) output(" Some part of you really hopes you don’t get cited for littering.");
	}
	clearMenu();
...
//steele birth scene
function ldcShockHopperSteeleBirth(numChildren = 2)
{
	clearOutput();
	author("William");
	clearBust();
	...
	let locType = 0;
	let multiPreg = false;
	let numWombs = flags["LDC_SHOCKHOPPER_MULTIBIRTH"];
	...
	else if (loc === "tavros station")
	{
		...
		if (!pc.isCrotchExposed()) output(" The medics relieve you of your clothes en-route.");
		moveToRoom("NURSERYE14");
		locType = 2;
	}
	//mhenga zil village 3
	else if (loc === "mhen’ga" && flags["PQ_RESOLUTION"] !== -1 && (getCurrentLocation() === "11 Zil Village Outskirts" || getCurrentLocation() === "12 Zil Village Winnar" || getCurrentLocation() === "ZIL FESTIVAL VILLAGE" || getCurrentLocation() === "ZIL FESTIVAL YURT"))
	{
	...
	//mhenga in civ 4
	else if (loc === "mhen’ga" && !InRoomWithFlag(RoomFlags.HAZARD))
	{
		...
		output(".");
		moveToRoom("ESBETH MEDICAL OFFICE");
		locType = 4;
	}
	//mhenga tarkus in civ 5
	else if (loc === "tarkus" && !InRoomWithFlag(RoomFlags.HAZARD))
	{
	...
	{
		let inKressia = false;
		if (isNaN(parseInt(getCurrentLocation())))
		{
			if (getCurrentLocation() === "LIEVE BUNKER" || getCurrentLocation() === "FAZIAN_RESCUE_ROOM" || getCurrentLocation() === "KRESSIA MEDICAL") inKressia = true;
		}
		else
		{
			if (parseInt(getCurrentLocation()) >= 800 && parseInt(getCurrentLocation()) <= 899) inKressia = true;
		}
		...
		if (inKressia)
		{
			...
			moveToRoom("KRESSIA MEDICAL");
			locType = 6;
		}
		//taivra 7
		else if (getCurrentLocation() === "2E11" || getCurrentLocation() === "2E13" || getCurrentLocation() === "2G13" || getCurrentLocation() === "2G15" || getCurrentLocation() === "2G17" || getCurrentLocation() === "2E17" || getCurrentLocation() === "2C11" || getCurrentLocation() === "2C13" || getCurrentLocation() === "2C15" || getCurrentLocation() === "2C09" || getCurrentLocation() === "2A09")
		{
		...
		{
			if (InRoomWithFlag(RoomFlags.HAZARD))
			{
	...
	//uveto kryms camp 9
	else if (loc === "uveto vii" && getCurrentLocation() === "UVGR M4")
	{
	...
	else if (loc === "uveto vii" && getCurrentLocation().search("KORGII") >= 0)
	{
	...
	//uveto town 11
	else if (loc === "uveto vii" && !InRoomWithFlag(RoomFlags.HAZARD))
	{
	...
	else if (loc === "zhèng shi station" && ratsPCIsGood() && (window.boredJumperPregLocation() === 0 || window.boredJumperPregLocation() === 1))
	{
		if (window.boredJumperPregLocation() === 0) output("The heat of the mines mixes with your weakness, makes the gush of cream at your loins all the more noticeable. A cold rush of adrenaline flows when you hear hasty footsteps.");
		else if (window.boredJumperPregLocation() === 1) output("Whilst you search for a safe place, a cold rush of adrenaline flows when the rapid ‘taptaptap’ of footsteps catches you off guard.");
	...
	//zheng shi mines 18
	else if (loc === "zhèng shi station" && window.boredJumperPregLocation() === 0)
	{
	...
	else if (loc === "zhèng shi station" && window.boredJumperPregLocation() === 1)
	{
	...
	//generic civ 20
	else if (loc !== "zhèng shi station" && !InRoomWithFlag(RoomFlags.HAZARD))
	{
	...
	if (flags["LDC_SHOCKHOPPER_PREGBY"] === 3 && multiPreg)
	{
		...
		output(" The pain you feel between your waist-expanding contractions is nothing compared to the sensations of when you drained Zhèng Shi’s top buns, when your sexual prowess put them in their rightful places.");
		...
		output(" whole litters are piled up in a sprawling pool of mewling munchkins, " + StringUtil.numToText(numChildren, true) + " laquine boys, girls, hermaphrodites, there’s every combination you could imagine! Some with little fur, some totally coated in it, but they’re all the offspring of your piratical tryst. All-black and blue and vanilla coats or limbs shimmer; eyes not even opened yet, they cry out quietly for you.");
	...
	else if (numChildren > 2 && numChildren <= 8)
	{
		...
		output("\n\nYou’ve given birth to " + StringUtil.numToText(numChildren) + " laquines, a variety of girls, boys, and natural hermaphrodites. Each one is painted with varying amounts of fur, pelts of mixed colors including <b>your [pc.hairColor] hair!</b>");
	...
	else if (numChildren > 9 && numChildren <= 16)
	{
		...
		output("\n\nYou pant, heave, and pant some more, finally able to sit up and take stock of your children. " + StringUtil.numToText(numChildren, true) + " laquines are huddled in a pile. Girls, boys, natural hermaphrodites, you’ve given birth to just about every combination of space-rabbit there is. Their furs are shimmery, if a little reddened with your innards. Some have full coats, others only have it on their limbs. The colors are all similar though;");
	...
	else if (numChildren >= 17)
	{
		...
		output("\n\nWhen you look down to the mound of squeals and twitching ears, your heart flutters. It feels so light that it might just be floating away to heaven, enraptured by the sight. Your " + StringUtil.numToText(numChildren) + " children all huddle together for warmth, each covered in a little bit of you. It’s no surprise you’ve just spawned every conceivable combination of laquine there is: boys, girls, hermaphrodites, lop-ears, upraised ears, furry limbs, totally furry, there’s almost too much to take in. Despite being marred by a little internal sheen,");
	...
	{
		output("\n\nYou give birth to " + StringUtil.numToText(numChildren) + " kids!");
	}
 ...
 * @param {Number} locType
 */
function ldcShockHopperSteeleBirthNext(numChildren, locType)
{
	clearOutput();
	author("William");
	clearBust();
	...
	let milk = 200 * numChildren;
	if (milk > pc.milkQ()) milk = pc.milkQ();

	if (window.boredJumperBreastFeedOK())
	{
	...
	else
	{
		...
		if (numChildren > 8) output(", one after the other");
